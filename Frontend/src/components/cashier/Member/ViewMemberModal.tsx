...
import { X, MapPin, Mail, Phone, User, Calendar, CreditCard, QrCode } from 'lucide-react';
        ...
        <div className="bg-[#14181f] rounded-2xl border border-gray-700/50 w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl shadow-red-500/10">
          ...
          <div className="flex items-center gap-6 mb-6">
            <div className="h-24 w-24 rounded-full bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center text-4xl font-bold text-white shadow-lg overflow-hidden ring-2 ring-red-500/30">
              {member.profile ? (
                <img src={`${STORAGE_URL}/${member.profile}`} alt="Profile" className="h-full w-full object-cover" />
              ) : (
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="bg-[#1e242c] p-3 rounded-lg border border-gray-700/30">
              ...
              <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${
                member.contract_status === 'active' ? 'bg-green-600/30 text-green-300' : member.contract_status === 'pending' ? 'bg-yellow-600/30 text-yellow-300' : 'bg-red-600/30 text-red-300'
              }`}>
          ...
          {/* QR Code Section with View QR Button */}
          <div className="mt-4 flex flex-col items-center bg-[#1e242c] p-4 rounded-lg border border-gray-700/30">
            <div className="flex items-center justify-between w-full mb-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white text-sm font-medium rounded-lg transition-all duration-300 shadow-lg shadow-red-500/20"
              >
                <QrCode size={16} />
                View QR Code
              </button>
