...
export const attendanceApi = {
    async getAttendances(filters: AttendanceFilters = {}) {
        ...
        const response = await api.get(`/cashier/attendance?${params.toString()}`);
        return response.data.data;
        },
  async getAttendance(id: number) {
    const response = await api.get(`/cashier/attendance/${id}`);
    return response.data.data;
  },

  async scanQR(qr_code: string) {
    const response = await api.post('/cashier/attendance/scan', { qr_code });
    return response.data;
  },

  async createAttendance(data: FormData) {
    const response = await api.post('/cashier/attendance', data, {
  ...
  async updateAttendance(id: number, data: FormData) {
    const response = await api.post(`/cashier/attendance/${id}?_method=PUT`, data, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return response.data.data;
  },
  ...
  async deleteAttendance(id: number) {
    await api.delete(`/cashier/attendance/${id}`);
  },
};